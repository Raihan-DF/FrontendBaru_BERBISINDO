...
import { BookOpen, Search, Calendar, Users, Zap, ChevronLeft, ChevronRight } from "lucide-react"
...
export default function StudentMaterials() {
      ...
      if (response.ok) {
        ...
        const materialsWithVideos = await Promise.all(
          materialsData.map(async (material: Material) => {
            try {
              const videoResponse = await fetch(buildUrl(`/api/materials/${material.id}/videos`), {
                headers: {
                  Authorization: `Bearer ${token}`,
                  Accept: "application/json",
                },
              ...
              console.error(`Error fetching videos for material ${material.id}:`, error)
              return { ...material, videos_count: 0 }
            }
          }),
        )
  ...
  const getFilteredMaterials = () => {
    ...
    switch (activeTab) {
      ...
      case "popular":
        return filteredMaterials.sort((a, b) => (b.videos_count || 0) - (a.videos_count || 0))
      default:
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-slate-900">
      <div className="container mx-auto py-4 px-3 sm:py-6 sm:px-4 space-y-4 sm:space-y-6">
        ...
        <div className="text-center space-y-2 sm:space-y-3">
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">Materi Pembelajaran</h1>
          <p className="text-sm sm:text-base text-gray-600 dark:text-gray-300 px-2">
            Pelajari berbagai materi bahasa isyarat dengan video pembelajaran interaktif
          </p>
        </div>
        ...
        <div className="rounded-xl bg-gradient-to-br from-[#06D7A0] to-[#108AB1] dark:from-[#073A4B] dark:to-[#0A3F52] p-4 sm:p-6 shadow-md text-center">
          <div className="flex flex-col items-center justify-center gap-2">
            <div className="text-4xl sm:text-5xl font-bold text-white dark:text-[#06D7A0]">{materials.length}</div>
            <p className="text-sm sm:text-base text-white dark:text-gray-300 font-medium">Total Materi Tersedia</p>
  ...
  if (materials.length === 0) {
    return (
      <div className="text-center py-12 bg-white dark:bg-slate-800 rounded-lg">
        ...
        <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">Tidak ada materi ditemukan</h3>
    ...
    <div className="bg-white dark:bg-slate-800 rounded-lg p-3 sm:p-4 shadow-sm border-l-4 border-l-[#06D7A0]">
      <div className="flex gap-3 sm:gap-4">
        ...
        <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-700 flex-shrink-0">
          {material.thumbnail ? (
            <img
              src={buildUrl(`/storage/${material.thumbnail || "/placeholder.svg"}`)}
              alt={material.title}
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between mb-2">
            ...
            <Badge className={`${getDifficultyColor(material.difficulty_level)} text-xs flex-shrink-0`}>
